...
void InfiniteLoopCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if (isa<ForStmt>(LoopStmt))
    LoopName = " `for` ";
  else if (isa<WhileStmt>(LoopStmt))
    LoopName = " `while` ";
  else if (isa<DoStmt>(LoopStmt))
    LoopName = " `do-while` ";

  const SourceManager &SM = Result.Context->getSourceManager();
  std::string msg = "Loop" + LoopName + "is infinite because of ";
  if (CondVarNames.empty()) {
    msg += "missing check in the loop condition";
  } else {
    msg += "no updates to condition variables (" + CondVarNames + ")";
  }
  registerIssue(msg, LoopStmt, SM);
}
