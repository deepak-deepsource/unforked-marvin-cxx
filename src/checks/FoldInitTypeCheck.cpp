...
void FoldInitTypeCheck::doCheck(const BuiltinType &IterValueType,
  ...
  if (!isValidBuiltinFold(IterValueType, InitType, Context)) {
    std::string msg = "Possible loss of precision due the type-folding from `" +
                      IterValueType.desugar().getAsString() + "` to `" +
                      InitType.desugar().getAsString() + "`";
    registerIssue(msg, CallNode, Context.getSourceManager());
  }
}
