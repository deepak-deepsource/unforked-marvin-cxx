...
#include "checks/UseNullptrCheck.h"
...
void replaceWithNullptr(CheckInterface &Check, SourceManager &SM,
...
public:
  CastSequenceVisitor(ASTContext &Context, ArrayRef<StringRef> NullMacros,
                      marvincxx::CheckInterface &Check)
  ...
  bool VisitStmt(Stmt *S) {
    ...
    if (SM.isMacroArgExpansion(StartLoc) && SM.isMacroArgExpansion(EndLoc)) {
      ...
      if (isReplaceableRange(FileLocStart, FileLocEnd, SM) &&
          allArgUsesValid(C)) {
        /// TODO: Add toml and issue code
        Check.registerIssue("", "use nullptr", C, SM);
      }
      return true;
    }
...
UseNullptrCheck::UseNullptrCheck(StringRef Name,
                                 marvincxx::AnalyserContext *Context)
    : CheckInterface(Name, Context), NullMacrosStr() {
